 ...
 */
public class ParallelSqrtBruteforceStrategy implements IPrimeGenerationStratregy {

    public List<Integer> generate(int toNumber) {
        return IntStream.rangeClosed(2, toNumber).parallel().filter(x -> this.isPrime(x)).boxed()
