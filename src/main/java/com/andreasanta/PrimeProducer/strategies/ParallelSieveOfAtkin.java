...
public class ParallelSieveOfAtkin implements IPrimeGenerationStratregy {
    ...
    public List<Integer> generate(int toNumber) {
        ...
        IntStream.range(1, equationRoot).parallel().forEach(x -> {
        ...
        IntStream.range(5, equationRoot).parallel().forEach(i -> {
            synchronized (primeList) {
                if (primeList.get(i)) {
